...
/**
 * @name GET /productionOrder/
 * @function
 * @memberof module:ProductionOrder
 * @description Home page con la generazione dell'interfaccia
 * @returns  Restituisce il file views/management/productionOrder.ejs renderizzato
 */
router.get('/',async  (req, res) => {
        let redirectId=req.query.redirectId?req.query.redirectId:null
        res.render("management/ProductionOrder",{redirectId:redirectId})

  });

/**
 * @name GET /productionOrder/getAll
 * @function
 * @memberof module:ProductionOrder
...
router.get('/getAll',async  (req, res) => {
    let productionOrder=await ProductionOrder.findAll({include:[
                                        {model: db.productionOrder},
                                    ]})   
    res.status(200).json({productionOrder:productionOrder})
});
